// pages/Amplifiers.jsx
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate, useLocation } from 'react-router-dom';
import AnimatedSection from '../components/AnimatedSection';
import { Amplifier } from '../data/amplifier';
import bgImage from "../../public/Brands/bg1.png";
// Brands to highlight on the Amplifiers page
const HIGHLIGHT_BRANDS = new Set(['Mesa', 'Fender', 'Park', 'Bird', 'Burns', 'Carlsbro']);
const Amplifiers = () => {
  ...
  const AmplifierTypes = [
    { id: 'electrique', name: 'Electric', count: 6 },
    { id: 'acoustique', name: 'Acoustic', count: 6 },
    { id: 'classique', name: 'Classical', count: 0 },
    { id: 'basse', name: 'Bass', count: 0 }
  ];

  const brands = [
    'Mesa', 'Fender', 'Park', 'Bird', 'Burns', 'Carlsbro'
  ];
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setTimeout(() => {
      setProducts(Amplifier);
      // if an id is present in query param, only display that product
      if (productIdParam) {
        const found = Amplifier.find(p => String(p.id) === String(productIdParam));
        setFilteredProducts(found ? [found] : []);
      } else {
        setFilteredProducts(Amplifier);
      }
      setLoading(false);
    }, 1000);
  }, []);
  ...
  const FiltersContent = () => (
    <>
      {/* Filter by type */}
      <div className="mb-8">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Amplifier type</h3>
        <div className="space-y-2">
          {AmplifierTypes.map(type => (
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Amplifiers...</p>
        </div>
      </div>
    );
  }
          ...
          <AnimatedSection>
            <div className="text-center max-w-3xl mx-auto relative z-10">
              <h1 className="text-4xl md:text-6xl font-bold mb-6 text-white">
                Our <span className="text-[#6819ce]">Amplifiers</span>
              </h1>
              <p className="text-xl text-white mb-8">
                Discover our exceptional selection of Amplifiers for every style and every level
              </p>
            </div>
          </AnimatedSection>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className="flex-1 w-full lg:max-w-md">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search for a Amplifier, a brand..."
                  value={searchTerm}
              ...
              <AnimatePresence mode="wait">
                {currentItems.length > 0 ? (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-2">No Amplifier found</h3>
  ...
  return (
    <AnimatedSection delay={index * 0.1}>
      ...
      >
        ...
        {/* Content */}
        <div className="p-6">
          {/* Brand with highlight badge for known amplifier brands */}
          <div className="flex items-center mb-2 gap-2">
            ...
            {HIGHLIGHT_BRANDS.has(product.brand) && (
              <span className="bg-[#6819ce] text-white px-2 py-0.5 rounded-full text-xs font-semibold">Featured</span>
            )}
          </div>
