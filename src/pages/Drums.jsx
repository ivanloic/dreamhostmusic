...
import { Drumskit } from '../data/Drumskit';
...
const Drums = () => {
  ...
  const guitarTypes = [
    { id: 'batterie-acoustique', name: 'acoustic drums', count: 6 },
    { id: 'batterie-electronique', name: 'electronic drums', count: 6 },
    { id: 'caisse-claire', name: 'snare drum', count: 0 },
    { id: 'machine-rythme', name: 'drum machine', count: 0 }
  ];

  const brands = [
    'Ludwig', 'Slingerland', 'Simmons', 'Camco', 'Asba', 'Rogers', 'Trixon', 'Premier', 'Meazzi'
  ];
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setTimeout(() => {
      setProducts(Drumskit);
      // if an id is present in query param, show only that product
      if (productIdParam) {
        const found = Drumskit.find(p => String(p.id) === String(productIdParam));
        setFilteredProducts(found ? [found] : []);
      } else {
        setFilteredProducts(Drumskit);
      }
      setLoading(false);
    }, 1000);
  }, []);
  ...
  const FiltersContent = () => (
    <>
      {/* Filter by type */}
      <div className="mb-8">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Drum type</h3>
  ...
  const handleQuickView = (product) => {
    ...
    // Here you can open a modal with details
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading drums...</p>
        </div>
      </div>
    );
  }
          ...
          <AnimatedSection>
            <div className="text-center max-w-3xl mx-auto relative z-10">
              <h1 className="text-4xl md:text-6xl font-bold mb-6 text-white">
                Our <span className="text-[#6819ce]">Drum Kits</span>
              </h1>
              <p className="text-xl text-white mb-8">
                Discover our exceptional selection of drum kits for all styles and all levels
              </p>
            </div>
          </AnimatedSection>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className="flex-1 w-full lg:max-w-md">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search for a drum kit, a brand..."
                  value={searchTerm}
              ...
              <AnimatePresence mode="wait">
                {currentItems.length > 0 ? (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-2">No drum kit found</h3>
  ...
  return (
    <AnimatedSection delay={index * 0.1}>
      <motion.div
        ...
        role="button"
        tabIndex={0}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/produit/${product.id}`, { state: { product } }); }}
      >
        {/* Product image */}
        ...
        <div className="p-6">
          {/* Brand */}
          <div className="text-[#008683] font-semibold text-xs sm:text-sm mb-2">{product.brand}</div>
